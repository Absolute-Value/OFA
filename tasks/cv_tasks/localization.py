...
from data.cv_data.localization_dataset import LocalizationDataset

logger = logging.getLogger(__name__)

@dataclass
class LocalizationConfig(OFAConfig):
    max_image_size: int = field(
        default=512, metadata={"help": "max image size for normalization"}
    )

@register_task("localization", dataclass=LocalizationConfig)
class LocalizationTask(OFATask):
    def __init__(self, cfg: LocalizationConfig, src_dict, tgt_dict):
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        self.datasets[split] = LocalizationDataset(
            split,
            dataset,
            self.bpe,
            self.src_dict,
