...
class HoiDataset(OFADataset):
    ...
    def process_image_text_pair(self, index): # 画像とテキストの読み込み
        uniq_id, image, caption, question, refs, gt_objects, dataset_name, type = self.dataset[index]

        image = Image.open(BytesIO(base64.urlsafe_b64decode(image))).convert("RGB") # 画像のバイナリデータをでコードして開き、RGBに変換
        patch_image = self.patch_resize_transform(image) if type != 'visual_grounding' else None # visual_groundingじゃなかったら
        ...
        if type == 'caption':
            ...
            src_item = self.encode_text(" what does the image describe?") # Image Captioning 入力
            tgt_item = self.encode_text(" {}".format(tgt_caption)) # Image Captioning 出力
            pos_src_item = self.encode_text(' does the image describe " {} "?'.format(pos_src_caption)) # ITM 入力
            ...
            src_item = self.encode_text(' which region does the text " {} " describe?'.format(src_caption)) # Visual Grounding 入力
            tgt_item = self.encode_text(region_coord, use_bpe=False) # Visual Grounding 出力
        else:
        ...
        if type == 'visual_grounding':
            ...
            region_prefix_item = self.encode_text('  what does the region describe? region:') # GC 入力
    ...
    def process_interaction(self, index):
        image_id, image, label = self.detection_dataset[index]
        image = Image.open(BytesIO(base64.urlsafe_b64decode(image))).convert("RGB")
        ...
        for label in label_list:
            # x0, y0, x1, y1, cat_id, cat = label.strip().split(',', 5)
            human_x0, human_y0, human_x1, human_y1, hoi_id, hoi, obj_x0, obj_y0, obj_x1, obj_y1, obj_id, obj = label.strip().split(',', 11)
        ...
        quant_boxes = []
        for i, (human_box, obj_box) in enumerate(boxes_target["human_boxes"], boxes_target["obj_boxes"]):
    ...
    def __getitem__(self, index):
        with data_utils.numpy_seed(self.seed, self.epoch):
            ...
            if self.split == 'train' and self.dataset.data_cnt % 8 == 0:
                ...
                extra_samples += self.process_interaction(0) if self.process_interaction else []
