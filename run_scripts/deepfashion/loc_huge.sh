...
log_dir=./logs/loc/huge/
save_dir=./checkpoints/loc/huge/
  ...
  for warmup_updates in {1000,}; do
    for lr in {5e-5,}; do
      for ix in ${!batch_size[@]}; do
        ...
        CUDA_VISIBLE_DEVICES="0,1" python -m torch.distributed.launch --nproc_per_node=${GPUS_PER_NODE} --nnodes=${WORKER_CNT} --node_rank=${RANK} --master_addr=${MASTER_ADDR} --master_port=${MASTER_PORT} ../../train.py \
          ${data} \
          --bpe-dir=${bpe_dir} \
          --user-dir=${user_dir} \
